...
class _DDEClient:
    ...
    def __init__(
        ...
        # 状态机：code → {field: value}，永不清空，只增量更新
        self._market_state: Dict[str, Dict[str, float]] = defaultdict(dict)
        self._state_lock = threading.Lock()
        # 待发送集合：记录本次 pump 轮次内有过更新的 code，由 _flush_dirty 统一发送
        self._dirty_codes: set = set()   # 仅在 pump 线程内访问，无需加锁
        ...
        try:
            ...
            while self._running:
                ...
                while u32.PeekMessageW(ctypes.byref(msg), None, 0, 0, 1):
                    u32.TranslateMessage(ctypes.byref(msg))
                    u32.DispatchMessageW(ctypes.byref(msg))
                # 泵送完本批消息后，统一发送脏 code 的快照（防止微观状态撕裂）
                self._flush_dirty()
                time.sleep(0.005)
    ...
    def _dde_callback(self, uType, uFmt, hconv, hsz1, hsz2, hdata, dw1, dw2):
        ...
        if uType == _XTYP_ADVDATA:
            try:
                ...
                if field and code:
                    value = self._read_advise_data(hdata)   # 可能为 None（BLANK/ERROR）
                    if "v" in field and value is not None:
                        value = float(int(round(value)))
                    self._update_state(code, field, value)  # None 也传入，由状态机处理哨兵
            except Exception:
                pass
            return _DDE_FACK
        return 0
    ...
    def _update_state(self, code: str, field: str, value: Optional[float]) -> None:
        """
        增量更新状态机，不触发 emit。
        value=None 表示 DDE 推送了 BLANK/ERROR，按字段类型写入哨兵值。
        """
        with self._state_lock:
            self._cb_total += 1
            if self._cb_total <= 5:
                logger.info("DDE 回调 #%d: code=%s field=%s value=%s",
                            self._cb_total, code, field, value)

            state = self._market_state[code]

            if value is None:
                # BLANK/ERROR 哨兵：绝不让 NaN 进入状态机
                if field == "ask1":
                    state["ask1"]  = 999999.0   # 无卖盘 → 极大值
                    state["askv1"] = 0.0
                elif field == "bid1":
                    state["bid1"]  = 0.0        # 无买盘 → 0
                    state["bidv1"] = 0.0
                elif field == "askv1":
                    state["askv1"] = 0.0        # 撤单 → 量清零
                elif field == "bidv1":
                    state["bidv1"] = 0.0        # 撤单 → 量清零
                # last 为 None 时不更新（保持上一笔真实值）
                else:
                    return
            else:
                state[field] = value

        # 标记该 code 有待发送数据（在 pump 线程内，无需锁）
        self._dirty_codes.add(code)

    def _flush_dirty(self) -> None:
        """
        将本轮 pump 内所有被更新的 code 以当前状态快照统一发送，
        避免单字段更新导致的价格/量错位（微观状态撕裂）。
        触发条件：状态机中 bid1 或 ask1 至少有一个已初始化（支持无成交合约做市报价）。
        """
        if not self._dirty_codes:
            return
        ts_ms = int(time.time() * 1000)
        for code in list(self._dirty_codes):
            with self._state_lock:
                state = dict(self._market_state.get(code, {}))
            # 触发条件：有任意盘口价格（不依赖 last，支持无成交远月合约）
            if "bid1" not in state and "ask1" not in state:
                continue
            snapshot = copy.deepcopy(state)
            self._on_tick_cb(code, snapshot, ts_ms)
            self._tick_total += 1
            if self._tick_total <= 3:
                logger.info("DDE tick #%d: code=%s fields=%s",
                            self._tick_total, code, list(snapshot.keys()))
