...
`action: SignalAction = SignalAction.OPEN`（或 `CLOSE`）区分开仓与平仓意图。`ArbitrageSignal` 对 OPEN/CLOSE 各有专用字段，**不再复用**：

| 字段 | OPEN 含义 | CLOSE 含义 |
|------|-----------|-----------|
| `etf_ask` | ETF 卖一价（买入 ETF） | 展示兜底 |
| `etf_bid` | 展示兜底 | ETF 买一价（卖出 ETF） |
| `put_ask` | Put 卖一价（买入 Put） | 展示兜底 |
| `put_bid` | 展示兜底 | Put 买一价（卖出 Put） |
| `call_bid` | Call 买一价（卖出 Call）| 展示兜底 |
| `call_ask` | 展示兜底 | Call 卖一价（买入 Call）|

Broker 按 `signal.action` 自动读取对应字段，**两套字段均在信号构造时填充，无语义翻转风险**。
