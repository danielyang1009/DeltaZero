...
- 职责：寻找定价偏差，生成套利信号；继承 `BaseStrategy`，必须实现 `generate_signals(self, snapshot: MarketSnapshot) -> Optional[ArbitrageSignal]`
- 规则：只放数学公式和逻辑判断，**绝对禁止**在策略内部维护任何状态

**`backtest/`（执行引擎与会计层）**
- `Broker`（`backtest/broker.py`）：`execute_signal` 入口按 `signal.action` 分派：OPEN 走四条微观机制校验（哨兵拦截 / 跨价撮合 / 容量限制 / 保证金前置校验）；CLOSE 走 `_execute_close`（滑点方向反转、无保证金校验、`margin_reserved=0`），均为 FOK 语义。
...
| `ArbitrageSignal` | `models/order.py` | Alpha Model 唯一输出，含净利润/盘口价格/辅助指标；`action="OPEN"\|"CLOSE"` 区分开平仓 |
| `TickAligner` | `data_engine/tick_aligner.py` | 有状态 LKV 容器，实盘/回测共用 |
| `PCPArbitrageStrategy` | `strategies/pcp_arbitrage.py` | 无状态策略；`scan_opportunities` 返回 OPEN 信号，`scan_close_opportunities` 返回 CLOSE 信号，`scan_pairs_for_display` 供监控展示 |
...
3. **套利计算**：`strategies/pcp_arbitrage.py` 的 `PCPArbitrageStrategy`（无状态）接收 `MarketSnapshot`；`scan_opportunities` 返回过阈值的 OPEN 信号，`scan_close_opportunities` 返回满足 `close_profit_threshold` 的 CLOSE 信号（Engine 负责按持仓过滤），`scan_pairs_for_display` 供监控无阈值全量展示；`_calc_forward_metrics()` / `_calc_close_metrics()` 为模块级纯函数，分别计算开仓/平仓净利润及辅助指标。
...
### ArbitrageSignal.action 字段语义

`action: str = "OPEN"` 区分开仓与平仓信号，**价格字段在 CLOSE 信号中含义翻转**：
...
`PCPArbitrageStrategy.scan_close_opportunities(snapshot, pairs)` 扫描全量配对，计算平仓净利润：
```
...
return open_signals + close_signals  # Engine 按 signal.action 自动分派
```
...
3. **计算入口**：核心计算必须全部在 `def generate_signals(self, snapshot: MarketSnapshot) -> Optional[ArbitrageSignal]`（或返回 `List[ArbitrageSignal]`）中完成。
