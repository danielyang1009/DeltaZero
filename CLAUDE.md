...
**`backtest/`（执行引擎与会计层）**
- `Broker/Engine`：撮合（看 ask1/bid1）、滑点计算、哨兵拦截（遇 999999.0 判废单）。**（注：目前作为权宜之计，这部分撮合逻辑暂存在 `Portfolio._execute_forward` 中，未来 Phase 5 将拆分为独立的 `broker.py`。新增代码时请遵循此现状）**
- `Portfolio`：纯会计，管钱（更新 cash）、管持仓、查可用资金；**禁止 `Portfolio` 直接看盘口撮合（未来目标）**。
...
| `ArbitrageSignal` | `models/order.py` | Alpha Model 唯一输出，含净利润/盘口价格/辅助指标 |
| `TickAligner` | `data_engine/tick_aligner.py` | 有状态 LKV 容器，实盘/回测共用 |
| `PCPArbitrageStrategy` | `strategies/pcp_arbitrage.py` | 无状态策略，`generate_signals(snapshot)` 或 `scan_pairs_for_display(snapshot, pairs)` |
...
3. **套利计算**：`strategies/pcp_arbitrage.py` 的 `PCPArbitrageStrategy`（无状态）接收 `MarketSnapshot`，`scan_pairs_for_display(snapshot, pairs)` 返回 `List[ArbitrageSignal]`，`_compute_forward_metrics()` 计算净利润及辅助指标（Max_Qty、SPRD、OBI、Net_1T、TOL）。
